...
def get_medications(disease):
    meds = medications[medications['Disease'] == disease].iloc[:, 1:].values.flatten()
    meds = [m for m in meds if str(m) != 'nan']
    return meds if len(meds) > 0 else ["No medications available"]

def get_workout(disease):
    wrk = workout[workout['disease'] == disease].iloc[:, 1:].values.flatten()
